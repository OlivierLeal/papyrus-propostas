...
class ProjectFinding < ApplicationRecord
  ...
  # afirma; CAL (o texto da lei promulgada, ver ProcessLegalNormsJob) vence qualquer documento
  # desta proposta — é a fonte primária, o que um TR ou ET dizem sobre a exigência é interpretação
  # de alguém; entre os documentos, TR (vem da própria instituição/órgão ambiental — autoridade
  # sobre os próprios requisitos) vence ET (pedido técnico do cliente — o que ele está pedindo, mas
  # não necessariamente a redação exata da exigência regulatória) vence complementar. Ordena o que
  # o consultor vê ao decidir uma divergência — nunca resolve a divergência sozinha.
  SOURCE_KINDS = {
    "consultor" => "Decisão do consultor",
    "sistema" => "Calculado pelo sistema",
    "cal" => "Legislação (CAL/Ius Natura)",
